...
Let $\mathbf{z}$ denote scar logits and $\mathbf{M}$ the myocardium mask. The constraint term is $\mathcal{L}_{\text{out}} = \frac{1}{HW} \sum_{i,j} \sigma(z_{i,j})(1 - M_{i,j})$, where $\sigma(\cdot)$ is the sigmoid function.
